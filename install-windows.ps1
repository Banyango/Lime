# install-windows.ps1
# Simple installer for Lime on Windows.
...
# Examples:
#   .\install-windows.ps1 -Repository "Banyango/lime"
#   .\install-windows.ps1 -Repository "Banyango/lime" -Asset "lime-windows-0.3.3.exe"
#   .\install-windows.ps1 -Repository "Banyango/lime" -Asset "https://github.com/owner/repo/releases/download/v0.3.3/lime-windows-0.3.3.exe"

param(
  [string]$Repository = "Banyango/lime",
  [string]$Asset = ''
)
...
$Asset = "lime-windows-$VERSION.exe"
...
if ($Asset -and ($Asset -like 'http*')) {
  ...
  if (-not $Asset) { $assetName = "lime-windows-$VERSION.exe" } else { $assetName = $Asset }
...
# Prefer Program Files\Lime if writable, else use LocalAppData\Programs\Lime, else fall back to %USERPROFILE%\bin
$installDir = Join-Path $env:ProgramFiles 'Lime'
if (-not (Test-Path $installDir)) {
  try { New-Item -ItemType Directory -Path $installDir -Force | Out-Null } catch { $installDir = $null }
}

if (-not $installDir) {
  $installDir = Join-Path $env:LOCALAPPDATA 'Programs\Lime'
...
$targetPath = Join-Path $installDir 'lime.exe'
...
Write-Host "Installed lime -> $targetPath"
...
Write-Host "Done. Run 'lime.exe --help' to verify."
